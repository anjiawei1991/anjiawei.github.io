...
* 输入了 `sudo docker run hello-world` 命令，然后期望 hello-world 镜像被运行。`hello-world` 为镜像名称，完整的镜像名称包括名字和 tag，比如 `hello-world:0.0.1`，如果没有指定 tag，那么 Docker 使用 `latest` 做为 tag 名。
...
488d2a203477  hello-world  "/hello"  2 minutes ago  Exited ...         friendly_babbage
```

这里展示的刚才运行完成的容器，是基于镜像 `hello-world` 创建的，并且容器名字是 docker 随机生成的 `friendly_babbage`(可以通过参数 `--name container-name` 可以自己指定容器名字)。
...
下面用 go 语言写一个简单的 http 服务程序，并且命名为 `main.go`，为每个访问的用户输出服务主机名以及服务程序启动命令:
...
为了把应用打包成镜像，首先要创建 Dockerfile 文件，Dockerfile 文件中包含了一系列构建镜像所需要的指令。
...
* `FROM golang:1.12` 表示我们需要基于 `golang:1.12` 来构建镜像，它定义了镜像的起始内容。`golang:1.12` 提供了 go 的编译工具。
* `WORKDIR /app` 命令将工作目录设置为 `/app`。
* `ADD main.go .` 将当前目录下的 main.go 文件拷贝到镜像中，目标位置是 WORKDIR 指定的 /app
...
通过使用 `docker image tag myapp user/myapp` 为镜像指定一个新的名字，然后就可以将镜像推送到官方的镜像仓库了: 
`docker push user/myapp`（在推送之前，你需要先在 https://hub.docker.com 创建账号，在这里，假设你的用户名为 `user`）
