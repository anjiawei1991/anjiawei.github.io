...
kubectl 从 `$HOME/.kube/config` 文件中获取集群的配置文件（这个配置文件中包含集群 API server 的地址、认证参数、上下文等信息），你也可以通过 KUBECONFIG 环境变量来指定配置文件（在使用 `kind` 搭建集群时将会用到）。

## 使用公共 playground

Kubernetes 的集群搭建并不是一件简单的工作。如果遇到麻烦，可以先使用一些公共的环境学会怎么使用，再去学怎么搭建环境。这里提供两个可以直接使用的 Kubernetes playground，你不需要搭建它们就能使用。

在这些 playgroud 中已经包含了 kubectl 工具，我们不需要再安装 kubectl。

1. <https://labs.play-with-k8s.com/>，这个网站上运行简单几个命令就可以搭建一个单节点的集群（在添加实例时，你会在终端上看到这些命令）：
...
`-o yaml` 的意思是用 `yaml` 格式输出资源的详细信息。可以看到这和我们创建 pod 时提供的描述文件格式相近，只不过多了一些信息。比如 metadata 中包括了 Pod 的 uid、creationTimestamp 等。另外最明显的区别是这例多了一个 `status` 段，它用来展示 pod 及其内部容器的信息、pod 所处的条件，所在节点以及 pod 的 IP 地址等信息。
...
在正式环境上，应该创建一个叫做 `Service` 的 Kubernetes 资源，然后通过它来访问服务。但是在这里，作者还不想介入 `Service`，所以使用了下面几种方式来访问服务：
...
由于我们的服务是监听在地址 0.0.0.0 上，所以我们可以通过 localhost 访问:
...
同时，由于服务的集群 ip 是 10.244.1.12，所以也可在容器内通过这个地址访问：
